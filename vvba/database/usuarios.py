...
def verificarClave(indiceUsuario, clave):
    ...
    '''
    return True
def guardarCredenciales(idUsuario, contraseña, listaEncriptacion):
    with open("passwords.csv", "a", encoding="utf-8") as archivinio:
        archivinio.write(f"{idUsuario};{contraseña};{listaEncriptacion}\n")

def leerCredenciales(idUsuario):
    with open("passwords.csv", "r", encoding="utf-8") as archivo:
        archivo.readline()
        for linea in archivo:
            linea = linea.strip()
            partes = linea.split(";")
            if int(partes[0]) == idUsuario:
                registro= {"idUsuario": partes[0], "contraseña": partes[1], "listaEncriptacion": partes[2]}
                return registro
    return None